#!/usr/bin/env python3
"""Enrich atomic_items.csv with metadata from master extract and custom lists.

Same enrichment as enrich_unmappable.py but for category-A (atomic) items.
Adds template/group/value-type/custom-list context to support clinical review.

Usage:
    uv run python -m fca.enrich_atomic
"""
import csv
...
def main():
    ...
    n_enriched = 0
    n_no_meta = 0
    with open(ATOMIC) as fin, open(OUTPUT, "w", newline="") as fout:
        reader = csv.DictReader(fin)
        writer = csv.writer(fout)
        writer.writerow([
            "source_concept_code", "source_description",
            "fca_concept_id", "fca_category", "fca_assessments", "fca_methods",
            "omop_domain", "suggested_concept_id", "suggested_concept_name",
            "val_type", "units", "min_val", "max_val",
        ...
        for row in reader:
            ...
            if templates or meta["val_type"]:
                n_enriched += 1
            else:
                n_no_meta += 1
            writer.writerow([
                rid, row["source_description"],
                row["fca_concept_id"], row["fca_category"],
                row.get("fca_assessments", ""), row.get("fca_methods", ""),
                row.get("omop_domain", ""),
                row.get("suggested_concept_id", ""),
                row.get("suggested_concept_name", ""),
    ...
    print(f"Total rows: {n_enriched + n_no_meta}")
    print(f"  With metadata: {n_enriched}")
    print(f"  No metadata:   {n_no_meta}")
